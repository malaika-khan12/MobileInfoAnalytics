...
type Health = { ok:true; repo_root?:string; database:{configured:boolean;reachable:boolean;error?:string}; scripts:Record<string,{path?:string;exists:boolean}>; jobs?:ControlJob[] };
...
export function AdminPanel() {
  ...
  return <AppShell footer><div className="page-container admin-page">
    ...
    {!auth?.authenticated&&<section className="panel"><div className="panel-heading"><div><span className="panel-kicker">Administrator authentication</span><h2>Unlock write operations</h2><p>{auth?.configured?"Enter MOBILE_ANALYTICS_ADMIN_TOKEN. The backend exchanges it for an HttpOnly SameSite session cookie.":"MOBILE_ANALYTICS_ADMIN_TOKEN is not configured on the Control API."}</p></div></div><form className="inline-form" onSubmit={login}><input type="password" autoComplete="current-password" value={token} onChange={event=>setToken(event.target.value)} placeholder="Operations token" disabled={!auth?.configured||busy==="login"}/><button className="button button-primary" disabled={!auth?.configured||!token||busy==="login"}>{busy==="login"?"Authenticating…":"Authenticate"}</button></form></section>}
